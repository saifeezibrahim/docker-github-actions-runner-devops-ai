...
if [[ -n "${REDACTED}" ]]; then
  ...
  if [[ -d "${REDACTED}" ]]; then
    ...
    cp -p -r "${REDACTED}/." "/actions-runner"
  fi
...
if [[ -n "${REDACTED}" ]]; then
  echo "Reusage is enabled. Storing data to ${REDACTED}"
  if [[ ${REDACTED} == "false" ]]; then
    echo "REDACTED should be set to true to avoid issues with re-using a deregistered runner."
    exit 1
  fi
  # Quoting (even with double-quotes) the regexp brokes the copying
  cp -p -r "/actions-runner/_diag" "/actions-runner/svc.sh" /actions-runner/.[^.]* "${REDACTED}"
fi
...
if [[ ${_DEBUG_ONLY} == "true" ]] || [[ ${_DEBUG_OUTPUT} == "true" ]] ; then
  echo ""
  echo "Disable automatic registration: ${REDACTED}"
...
if [[ ${_RUN_AS_ROOT} == "true" ]]; then
  ...
  if [[ $(id -u) -eq 0 ]]; then
    [[ -n "${REDACTED}" ]] && chown -R runner "${REDACTED}"
